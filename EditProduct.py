...
class EditProduct(ctk.CTk):
    def __init__(self, product):
        ...
        self.create_edit_product_screen()
        self.mainloop()
    ...
    def create_edit_product_screen(self):
        ...
        # Przycisk Dodaj
        self.add_button = ctk.CTkButton(self, text="Dodaj", command=self.edit_product)
    ...
    def edit_product(self):
        ...
        print("Edytowanie produktu:", data)
        name = data["nazwa"]
        quantity = data["ilość"]
        ...
        try:
            ...
            with sqlite3.connect(db_path) as conn:
                ...
                if category:
                    ...
                    print(category[0])
                    print("Kategoria istnieje")
                    print(category)

                    # Zapytanie SQL do wstawienia produktu
                cursor.execute("""
                    UPDATE products
                    SET category_id = ?, name = ?, description = ?, price = ?, stock_quantity = ?, updated_at = CURRENT_TIMESTAMP
                    WHERE product_id = ?
                """, (category_id, name, description, float(price), int(quantity), self.product_id))
                print("Produkt zaktualizowany w bazie danych.")

                # Zatwierdzenie zmian w bazie
                conn.commit()
                print("Produkt zedytowany w bazie danych!")
